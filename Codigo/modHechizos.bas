...
10  On Error GoTo InfoHechizo_Err

20  With UserList(UserIndex)

30      h = .Stats.UserHechizos(.flags.Hechizo)
40      SkinSpell = .Stats.UserSkinsHechizos(h)

50      If SkinSpell > 0 Then
60          h = 1
70      End If

80      If .flags.NoPalabrasMagicas = 0 Then
90          Call DecirPalabrasMagicas(h, UserIndex)
100     End If

110     If IsValidUserRef(.flags.TargetUser) Then    '¿El Hechizo fue tirado sobre un usuario?
120         If Hechizos(h).FXgrh > 0 Then    '¿Envio FX?
130             If Hechizos(h).ParticleViaje > 0 Then
140                 UserList(.flags.TargetUser.ArrayIndex).Counters.timeFx = 3
150                 Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, UserList(.flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, Hechizos(h).TimeParticula, Hechizos(h).wav, 1, UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
160             Else
170                 UserList(.flags.TargetUser.ArrayIndex).Counters.timeFx = 3

180                 If .Stats.UserSkinsHechizos(h) > 0 Then
190                     Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageCreateFX(UserList(.flags.TargetUser.ArrayIndex).Char.charindex, .Stats.UserSkinsHechizos(h), Hechizos(h).loops, UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
200                 Else
210                     Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageCreateFX(UserList(.flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).FXgrh, Hechizos(h).loops, UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
220                 End If
230             End If
240         End If

250         If Hechizos(h).Particle > 0 Then    '¿Envio Particula?
260             If Hechizos(h).ParticleViaje > 0 Then
270                 UserList(.flags.TargetUser.ArrayIndex).Counters.timeFx = 3
280                 Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, UserList(.flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0, UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
290             Else
300                 UserList(.flags.TargetUser.ArrayIndex).Counters.timeFx = 3
310                 Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageParticleFX(UserList(.flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).Particle, Hechizos(h).TimeParticula, False, , UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
320             End If
330         End If

340         If Hechizos(h).ParticleViaje = 0 Then
350             Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessagePlayWave(Hechizos(h).wav, UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
360         End If

370         If Hechizos(h).TimeEfect <> 0 Then    'Envio efecto de screen
380             Call WriteFlashScreen(UserIndex, Hechizos(h).ScreenColor, Hechizos(h).TimeEfect)
390         End If

400     ElseIf IsValidNpcRef(.flags.TargetNPC) Then    '¿El Hechizo fue tirado sobre un npc?

410         If Hechizos(h).FXgrh > 0 Then    '¿Envio FX?
420             If NpcList(.flags.TargetNPC.ArrayIndex).Stats.MinHp < 1 Then
...
430                 If Hechizos(h).ParticleViaje > 0 Then
440                     Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestinoXY(.Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, Hechizos(h).TimeParticula, Hechizos(h).wav, 1, .flags.TargetX, .flags.TargetY))
450                 Else
460                     If .Stats.UserSkinsHechizos(h) > 0 Then
470                         Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageFxPiso(.Stats.UserSkinsHechizos(h), .flags.TargetX, .flags.TargetY))
480                     Else
490                         Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageFxPiso(Hechizos(h).FXgrh, .flags.TargetX, .flags.TargetY))
500                     End If
510                 End If
520             Else
530                 If Hechizos(h).ParticleViaje > 0 Then
540                     Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, NpcList(.flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, Hechizos(h).TimeParticula, Hechizos(h).wav, 1))
550                 Else
560                     Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageCreateFX(NpcList(.flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).FXgrh, Hechizos(h).loops))
570                 End If
580             End If
590         End If

600         If Hechizos(h).Particle > 0 Then    '¿Envio Particula?
610             If NpcList(.flags.TargetNPC.ArrayIndex).Stats.MinHp < 1 Then
620                 Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestinoXY(.Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0, NpcList(.flags.TargetNPC.ArrayIndex).pos.x, NpcList(.flags.TargetNPC.ArrayIndex).pos.y))
630             Else
640                 If Hechizos(h).ParticleViaje > 0 Then
650                     Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, NpcList(.flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0))
660                 Else
670                     Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFX(NpcList(.flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).Particle, Hechizos(h).TimeParticula, False))
680                 End If
690             End If
700         End If

710         If Hechizos(h).ParticleViaje = 0 Then
720             Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessagePlayWave(Hechizos(h).wav, NpcList(.flags.TargetNPC.ArrayIndex).pos.x, NpcList(.flags.TargetNPC.ArrayIndex).pos.y))
730         End If
740     Else                   ' Entonces debe ser sobre el terreno
750         If Hechizos(h).FXgrh > 0 Then    'Envio Fx?

760             If .Stats.UserSkinsHechizos(h) > 0 Then
770                 Call modSendData.SendToAreaByPos(.pos.Map, .flags.TargetX, .flags.TargetY, PrepareMessageFxPiso(.Stats.UserSkinsHechizos(h), .flags.TargetX, .flags.TargetY))
780             Else
790                 Call modSendData.SendToAreaByPos(.pos.Map, .flags.TargetX, .flags.TargetY, PrepareMessageFxPiso(Hechizos(h).FXgrh, .flags.TargetX, .flags.TargetY))
800             End If

810             Call modSendData.SendToAreaByPos(.pos.Map, .flags.TargetX, .flags.TargetY, PrepareMessageFxPiso(Hechizos(h).FXgrh, .flags.TargetX, .flags.TargetY))
820         End If

830         If Hechizos(h).Particle > 0 Then    'Envio Particula?
840             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageParticleFXToFloor(.flags.TargetX, .flags.TargetY, Hechizos(h).Particle, Hechizos(h).TimeParticula))
850         End If

860         If Hechizos(h).wav <> 0 Then
870             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessagePlayWave(Hechizos(h).wav, .flags.TargetX, .flags.TargetY))    'Esta linea faltaba. Pablo (ToxicWaste)
880         End If

890     End If

900     If .ChatCombate = 1 Then
910         If Hechizos(h).Target = e_TargetType.uTerreno Then
920             Call WriteConsoleMsg(UserIndex, "ProMSG*" & h, e_FontTypeNames.FONTTYPE_FIGHT)

930         ElseIf IsValidUserRef(.flags.TargetUser) Then
                'Optimizacion de protocolo por Ladder
940             If UserIndex <> .flags.TargetUser.ArrayIndex Then
950                 Call WriteConsoleMsg(UserIndex, "HecMSGU*" & h & "*" & UserList(.flags.TargetUser.ArrayIndex).name, e_FontTypeNames.FONTTYPE_FIGHT)
960                 Call WriteConsoleMsg(.flags.TargetUser.ArrayIndex, "HecMSGA*" & h & "*" & .name, e_FontTypeNames.FONTTYPE_FIGHT)
970             Else
980                 Call WriteConsoleMsg(UserIndex, "ProMSG*" & h, e_FontTypeNames.FONTTYPE_FIGHT)
990             End If

1000        ElseIf .flags.TargetNPC.ArrayIndex > 0 Then
1010            Call WriteConsoleMsg(UserIndex, "HecMSG*" & h, e_FontTypeNames.FONTTYPE_FIGHT)
1020        Else
1030            Call WriteConsoleMsg(UserIndex, "ProMSG*" & h, e_FontTypeNames.FONTTYPE_FIGHT)
1040        End If
1050    End If
...
InfoHechizo_Err:
1080 Call TraceError(Err.Number, Err.Description, "modHechizos.InfoHechizo", Erl)
