...
Private Function PuedeLanzar(ByVal UserIndex As Integer, ByVal HechizoIndex As Integer, Optional ByVal Slot As Integer = 0) As Boolean
    ...
    With UserList(UserIndex)
        ...
        If .Stats.MinHp < Hechizos(HechizoIndex).RequiredHP Then
            ...
            Exit Function
        End If
        If .Stats.MinMAN < ManaHechizoPorClase(UserIndex, Hechizos(HechizoIndex), HechizoIndex) Then
...
Sub HandleHechizoUsuario(ByVal UserIndex As Integer, ByVal uh As Integer)
    ...
    Select Case Hechizos(uh).Tipo
        ...
        Case e_TipoHechizo.uPhysicalSkill
            b = HandlePhysicalSkill(UserIndex, eUser, UserList(UserIndex).flags.TargetUser.ArrayIndex, eUser, UserList(UserIndex).Stats.UserHechizos(UserList( _
                    UserIndex).flags.Hechizo), IsAlive)
    End Select
    If b Then
...
Sub HandleHechizoNPC(ByVal UserIndex As Integer, ByVal uh As Integer)
    ...
    Select Case Hechizos(uh).Tipo
        Case e_TipoHechizo.uEstado ' Afectan estados (por ejem : Envenenamiento)
            Call HechizoEstadoNPC(UserList(UserIndex).flags.TargetNPC.ArrayIndex, uh, b, UserIndex)
        Case e_TipoHechizo.uPropiedades ' Afectan HP,MANA,STAMINA,ETC
            Call HechizoPropNPC(uh, UserList(UserIndex).flags.TargetNPC.ArrayIndex, UserIndex, b, IsAlive)
        Case e_TipoHechizo.uPhysicalSkill
            b = HandlePhysicalSkill(UserIndex, eUser, UserList(UserIndex).flags.TargetNPC.ArrayIndex, eNpc, UserList(UserIndex).Stats.UserHechizos(UserList( _
                    UserIndex).flags.Hechizo), IsAlive)
    End Select
    If b Then
        If Hechizos(uh).EotId > 0 And IsAlive Then
...
Sub HechizoEstadoUsuario(ByVal UserIndex As Integer, ByRef b As Boolean)
    ...
    If IsSet(Hechizos(h).Effects, e_SpellEffects.Invisibility) Then
        ...
        If UserList(UserIndex).flags.Montado Then
            ...
            Exit Sub
        End If
        If UserList(tU).flags.Montado Then
        ...
        If MapInfo(UserList(tU).pos.Map).SinInviOcul Then
            ...
            Exit Sub
        End If
        If UserList(tU).flags.invisible = 1 Or UserList(tU).Counters.DisabledInvisibility > 0 Then
        ...
        Call SendData(SendTarget.ToPCArea, tU, PrepareMessageSetInvisible(UserList(tU).Char.charindex, True, UserList(tU).pos.x, UserList(tU).pos.y))
        Call InfoHechizo(UserIndex)
        b = True
    End If
    ...
    If Hechizos(h).Mimetiza = 1 Then
        ...
        End If
        'Si sos user, no uses este hechizo con GMS.
        If Not EsGM(UserIndex) And EsGM(tU) Then Exit Sub
        ...
        With UserList(UserIndex)
            ...
            .Char.CartAnim = UserList(tU).Char.CartAnim
    ...
    If Hechizos(h).Envenena > 0 Then
        ...
        If UserList(tU).flags.Envenenado = 0 Then
            ...
            UserList(tU).Counters.Veneno = Hechizos(h).Duration
            Call InfoHechizo(UserIndex)
            b = True
        Else
            Call WriteConsoleMsg(UserIndex, PrepareMessageLocaleMsg(1870, UserList(tU).name, e_FontTypeNames.FONTTYPE_INFO)) ' Msg1870=¬1 ya está envenenado. El hechizo no tuvo efecto.
            b = False
        End If
    End If
    ...
    If IsSet(Hechizos(h).Effects, e_SpellEffects.CurePoison) Then
        ...
        If UserList(tU).flags.Envenenado = 0 Then
            Call WriteConsoleMsg(UserIndex, PrepareMessageLocaleMsg(1871, UserList(tU).name, e_FontTypeNames.FONTTYPE_INFOIAO)) ' Msg1871=¬1 no está envenenado, el hechizo no tiene efecto.
            b = False
            Exit Sub
        End If
    ...
    If IsSet(Hechizos(h).Effects, e_SpellEffects.Immobilize) Then
        ...
        End If
        If UserList(tU).Counters.TiempoDeInmunidadParalisisNoMagicas > 0 Then
            Call WriteConsoleMsg(UserIndex, PrepareMessageLocaleMsg(1873, UserList(tU).name, e_FontTypeNames.FONTTYPE_FIGHT)) ' Msg1873=¬1 no puede volver a ser inmovilizado tan rápido.
            Exit Sub
        End If
        If IsSet(UserList(tU).flags.StatusMask, eCCInmunity) Then
            Call WriteLocaleMsg(UserIndex, MsgCCInunity, e_FontTypeNames.FONTTYPE_FIGHT)
        ...
        If IsValidUserRef(.flags.TargetUser) Then    '¿El Hechizo fue tirado sobre un usuario?
            If Hechizos(h).FXgrh > 0 Then    '¿Envio FX?
                If Hechizos(h).ParticleViaje > 0 Then
                    ...
                    Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, UserList( _
                            .flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, Hechizos(h).TimeParticula, Hechizos(h).wav, 1, UserList( _
                            .flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
                Else
                    UserList(.flags.TargetUser.ArrayIndex).Counters.timeFx = 3
                    If .Stats.UserSkinsHechizos(h) > 0 Then
                        Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageCreateFX(UserList(.flags.TargetUser.ArrayIndex).Char.charindex, _
                                .Stats.UserSkinsHechizos(h), Hechizos(h).loops, UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
                    Else
                        Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageCreateFX(UserList(.flags.TargetUser.ArrayIndex).Char.charindex, _
                                Hechizos(h).FXgrh, Hechizos(h).loops, UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
                    End If
                End If
            End If
            If Hechizos(h).Particle > 0 Then    '¿Envio Particula?
                If Hechizos(h).ParticleViaje > 0 Then
                    UserList(.flags.TargetUser.ArrayIndex).Counters.timeFx = 3
                    Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, UserList( _
                            .flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0, _
                            UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
                Else
                    UserList(.flags.TargetUser.ArrayIndex).Counters.timeFx = 3
                    Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessageParticleFX(UserList(.flags.TargetUser.ArrayIndex).Char.charindex, _
                            Hechizos(h).Particle, Hechizos(h).TimeParticula, False, , UserList(.flags.TargetUser.ArrayIndex).pos.x, UserList(.flags.TargetUser.ArrayIndex).pos.y))
                End If
            End If
            If Hechizos(h).ParticleViaje = 0 Then
                Call SendData(SendTarget.ToPCAliveArea, .flags.TargetUser.ArrayIndex, PrepareMessagePlayWave(Hechizos(h).wav, UserList(.flags.TargetUser.ArrayIndex).pos.x, _
                        UserList(.flags.TargetUser.ArrayIndex).pos.y))
            End If
        ...
        ElseIf IsValidNpcRef(.flags.TargetNPC) Then    '¿El Hechizo fue tirado sobre un npc?
            If Hechizos(h).FXgrh > 0 Then    '¿Envio FX?
                If NpcList(.flags.TargetNPC.ArrayIndex).Stats.MinHp < 1 Then
                    ...
                    If Hechizos(h).ParticleViaje > 0 Then
                        Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestinoXY(.Char.charindex, Hechizos(h).ParticleViaje, _
                                Hechizos(h).FXgrh, Hechizos(h).TimeParticula, Hechizos(h).wav, 1, .flags.TargetX, .flags.TargetY))
                    Else
                        If .Stats.UserSkinsHechizos(h) > 0 Then
                ...
                Else
                    If Hechizos(h).ParticleViaje > 0 Then
                        Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, NpcList( _
                                .flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, Hechizos(h).TimeParticula, Hechizos(h).wav, 1))
                    Else
                        Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageCreateFX(NpcList(.flags.TargetNPC.ArrayIndex).Char.charindex, _
            ...
            If Hechizos(h).Particle > 0 Then    '¿Envio Particula?
                If NpcList(.flags.TargetNPC.ArrayIndex).Stats.MinHp < 1 Then
                    Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestinoXY(.Char.charindex, Hechizos(h).ParticleViaje, _
                            Hechizos(h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0, NpcList(.flags.TargetNPC.ArrayIndex).pos.x, NpcList( _
                            .flags.TargetNPC.ArrayIndex).pos.y))
                Else
                    If Hechizos(h).ParticleViaje > 0 Then
                        Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestino(.Char.charindex, NpcList( _
                                .flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0))
                    Else
                        Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessageParticleFX(NpcList(.flags.TargetNPC.ArrayIndex).Char.charindex, _
                                Hechizos(h).Particle, Hechizos(h).TimeParticula, False))
                    End If
                End If
            End If
            If Hechizos(h).ParticleViaje = 0 Then
                Call SendData(SendTarget.ToNPCAliveArea, .flags.TargetNPC.ArrayIndex, PrepareMessagePlayWave(Hechizos(h).wav, NpcList(.flags.TargetNPC.ArrayIndex).pos.x, NpcList( _
                        .flags.TargetNPC.ArrayIndex).pos.y))
            End If
...
Sub HechizoCombinados(ByVal UserIndex As Integer, ByRef b As Boolean, ByRef IsAlive As Boolean)
    ...
    ElseIf Hechizos(h).SubeAgilidad = 2 Then
        ...
        Damage = RandomNumber(Hechizos(h).MinAgilidad, Hechizos(h).MaxAgilidad)
        UserList(tempChr).flags.DuracionEfecto = Hechizos(h).Duration
        If UserList(tempChr).Stats.UserAtributos(e_Atributos.Agilidad) - Damage < 6 Then
            UserList(tempChr).Stats.UserAtributos(e_Atributos.Agilidad) = MINATRIBUTOS
        Else
            UserList(tempChr).Stats.UserAtributos(e_Atributos.Agilidad) = UserList(tempChr).Stats.UserAtributos(e_Atributos.Agilidad) - Damage
        End If
    ...
    ElseIf Hechizos(h).SubeFuerza = 2 Then
        ...
        Damage = RandomNumber(Hechizos(h).MinFuerza, Hechizos(h).MaxFuerza)
        UserList(tempChr).flags.DuracionEfecto = Hechizos(h).Duration
        If UserList(tempChr).Stats.UserAtributos(e_Atributos.Fuerza) - Damage < 6 Then
            UserList(tempChr).Stats.UserAtributos(e_Atributos.Fuerza) = MINATRIBUTOS
        Else
            UserList(tempChr).Stats.UserAtributos(e_Atributos.Fuerza) = UserList(tempChr).Stats.UserAtributos(e_Atributos.Fuerza) - Damage
        End If
    ...
    If IsSet(Hechizos(h).Effects, e_SpellEffects.eDoHeal) Then
        ...
        Call UserMod.DoDamageOrHeal(tempChr, UserIndex, eUser, Damage, e_DamageSourceType.e_magic, h)
        If UserIndex <> tempChr Then
            Call WriteConsoleMsg(UserIndex, PrepareMessageLocaleMsg(1918, Damage & "¬" & UserList(tempChr).name, e_FontTypeNames.FONTTYPE_FIGHT)) ' Msg1918=Le has restaurado ¬1 puntos de vida a ¬2.
            Call WriteConsoleMsg(tempChr, PrepareMessageLocaleMsg(1919, UserList(UserIndex).name & "¬" & Damage, e_FontTypeNames.FONTTYPE_FIGHT)) ' Msg1919=¬1 te ha restaurado ¬2 puntos de vida.
        Else
