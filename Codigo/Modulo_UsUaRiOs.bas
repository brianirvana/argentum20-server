...
10  On Error GoTo Complete_ConnectUser_Err

20  ConnectUser_Complete = False

    Dim n                       As Integer
    Dim tStr                    As String
30  Call SendData(SendTarget.ToIndex, UserIndex, PrepareActiveToggles)
40  With UserList(UserIndex)

50      If .flags.Paralizado = 1 Then
60          .Counters.Paralisis = IntervaloParalizado
70      End If

80      If .flags.Muerto = 0 Then
90          .Char = .OrigChar

100         If .Char.body = 0 Then
110             Call SetNakedBody(UserList(UserIndex))
120         End If

130         If .Char.head = 0 Then
140             .Char.head = 1
150         End If
160     Else
170         .Char.body = iCuerpoMuerto
180         .Char.head = iCabezaMuerto
190         .Char.WeaponAnim = NingunArma
200         .Char.ShieldAnim = NingunEscudo
210         .Char.CascoAnim = NingunCasco
220         .Char.CartAnim = NoCart
230         .Char.Heading = e_Heading.SOUTH
240     End If

250     .Stats.UserAtributos(e_Atributos.Fuerza) = 18 + ModRaza(.raza).Fuerza
260     .Stats.UserAtributos(e_Atributos.Agilidad) = 18 + ModRaza(.raza).Agilidad
270     .Stats.UserAtributos(e_Atributos.Inteligencia) = 18 + ModRaza(.raza).Inteligencia
280     .Stats.UserAtributos(e_Atributos.Constitucion) = 18 + ModRaza(.raza).Constitucion
290     .Stats.UserAtributos(e_Atributos.Carisma) = 18 + ModRaza(.raza).Carisma

300     .Stats.UserAtributosBackUP(e_Atributos.Fuerza) = .Stats.UserAtributos(e_Atributos.Fuerza)
310     .Stats.UserAtributosBackUP(e_Atributos.Agilidad) = .Stats.UserAtributos(e_Atributos.Agilidad)
320     .Stats.UserAtributosBackUP(e_Atributos.Inteligencia) = .Stats.UserAtributos(e_Atributos.Inteligencia)
330     .Stats.UserAtributosBackUP(e_Atributos.Constitucion) = .Stats.UserAtributos(e_Atributos.Constitucion)
340     .Stats.UserAtributosBackUP(e_Atributos.Carisma) = .Stats.UserAtributos(e_Atributos.Carisma)

350     .Stats.MaxMAN = UserMod.GetMaxMana(UserIndex)
360     .Stats.MaxSta = UserMod.GetMaxStamina(UserIndex)
370     .Stats.MinHIT = UserMod.GetHitModifier(UserIndex) + 1
380     .Stats.MaxHit = UserMod.GetHitModifier(UserIndex) + 2
390     .Stats.MinHp = Min(.Stats.MinHp, .Stats.MaxHp)
400     .Stats.MinMAN = Min(.Stats.MinMAN, UserMod.GetMaxMana(UserIndex))

        'Obtiene el indice-objeto del arma
410     If .invent.EquippedWeaponSlot > 0 Then
420         If .invent.Object(.invent.EquippedWeaponSlot).ObjIndex > 0 Then
430             .invent.EquippedWeaponObjIndex = .invent.Object(.invent.EquippedWeaponSlot).ObjIndex

440             If .flags.Muerto = 0 Then
450                 .Char.Arma_Aura = ObjData(.invent.EquippedWeaponObjIndex).CreaGRH
460             End If

470             If .Invent_Skins.SlotWeaponEquipped > 0 Then
480                 If .Invent_Skins.Object(.Invent_Skins.SlotWeaponEquipped).ObjIndex = .Invent_Skins.ObjIndexWeaponEquipped And .Invent_Skins.ObjIndexWeaponEquipped > 0 Then
490                     If CanEquipSkin(UserIndex, .Invent_Skins.SlotWeaponEquipped, e_OBJType.otSkinsWeapons, False) Then
500                         Call SkinEquip(UserIndex, .Invent_Skins.SlotWeaponEquipped, .Invent_Skins.Object(.Invent_Skins.SlotWeaponEquipped).ObjIndex, e_OBJType.otSkinsWeapons)
510                     End If
520                 End If
530             End If
540         Else
550             .invent.EquippedWeaponSlot = 0
560         End If
570     End If

        ' clear hotkey settings, the client should set this
580     For n = 0 To HotKeyCount - 1
590         .HotkeyList(n).Index = -1
600         .HotkeyList(n).LastKnownSlot = -1
610         .HotkeyList(n).Type = Unknown
620     Next n

        'Obtiene el indice-objeto del armadura
630     If .invent.EquippedArmorSlot > 0 Then
640         If .invent.Object(.invent.EquippedArmorSlot).ObjIndex > 0 Then

650             .invent.EquippedArmorObjIndex = .invent.Object(.invent.EquippedArmorSlot).ObjIndex

660             If .flags.Muerto = 0 Then
670                 .Char.Body_Aura = ObjData(.invent.EquippedArmorObjIndex).CreaGRH
680             End If

690             If .Invent_Skins.SlotArmourEquipped > 0 Then
700                 If .Invent_Skins.Object(.Invent_Skins.SlotArmourEquipped).ObjIndex = .Invent_Skins.ObjIndexArmourEquipped And .Invent_Skins.ObjIndexArmourEquipped > 0 Then
710                     If CanEquipSkin(UserIndex, .Invent_Skins.SlotArmourEquipped, e_OBJType.otSkinsArmours, False) Then
720                         Call SkinEquip(UserIndex, .Invent_Skins.SlotArmourEquipped, .Invent_Skins.Object(.Invent_Skins.SlotArmourEquipped).ObjIndex, e_OBJType.otSkinsArmours)
730                     End If
740                 End If
750             End If
760         Else
770             .invent.EquippedArmorSlot = 0
780         End If
790         .flags.Desnudo = 0
800     Else
810         .flags.Desnudo = 1
...
830     If .invent.EquippedShieldSlot > 0 Then
840         If .invent.Object(.invent.EquippedShieldSlot).ObjIndex > 0 Then
850             .invent.EquippedShieldObjIndex = .invent.Object(.invent.EquippedShieldSlot).ObjIndex

860             If .flags.Muerto = 0 Then
870                 .Char.Escudo_Aura = ObjData(.invent.EquippedShieldObjIndex).CreaGRH
880             End If

890             If .Invent_Skins.SlotShieldEquipped > 0 Then
900                 If .Invent_Skins.Object(.Invent_Skins.SlotShieldEquipped).ObjIndex = .Invent_Skins.ObjIndexShieldEquipped And .Invent_Skins.ObjIndexShieldEquipped > 0 Then
910                     If CanEquipSkin(UserIndex, .Invent_Skins.SlotShieldEquipped, e_OBJType.otSkinsShields, False) Then
920                         Call SkinEquip(UserIndex, .Invent_Skins.SlotShieldEquipped, .Invent_Skins.Object(.Invent_Skins.SlotShieldEquipped).ObjIndex, e_OBJType.otSkinsShields)
930                     End If
940                 End If
950             End If

960         Else
970             .invent.EquippedShieldSlot = 0
980         End If
990     End If

        'Obtiene el indice-objeto del casco
1000    If .invent.EquippedHelmetSlot > 0 Then
1010        If .invent.Object(.invent.EquippedHelmetSlot).ObjIndex > 0 Then
1020            .invent.EquippedHelmetObjIndex = .invent.Object(.invent.EquippedHelmetSlot).ObjIndex

1030            If .flags.Muerto = 0 Then
1040                .Char.Head_Aura = ObjData(.invent.EquippedHelmetObjIndex).CreaGRH
1050            End If

1060            If .Invent_Skins.SlotHelmetEquipped > 0 Then
1070                If .Invent_Skins.Object(.Invent_Skins.SlotHelmetEquipped).ObjIndex = .Invent_Skins.ObjIndexHelmetEquipped And .Invent_Skins.ObjIndexHelmetEquipped > 0 Then
1080                    If CanEquipSkin(UserIndex, .Invent_Skins.SlotHelmetEquipped, e_OBJType.otSkinsHelmets, False) Then
1090                        Call SkinEquip(UserIndex, .Invent_Skins.SlotHelmetEquipped, .Invent_Skins.Object(.Invent_Skins.SlotHelmetEquipped).ObjIndex, e_OBJType.otSkinsHelmets)
1100                    End If
1110                End If
1120            End If

1130        Else
1140            .invent.EquippedHelmetSlot = 0
1150        End If
1160    End If

        'Obtiene el indice-objeto barco
1170    If .invent.EquippedShipSlot > 0 Then
1180        If .invent.Object(.invent.EquippedShipSlot).ObjIndex > 0 Then
1190            .invent.EquippedShipObjIndex = .invent.Object(.invent.EquippedShipSlot).ObjIndex

1200            If .Invent_Skins.ObjIndexBoatEquipped > 0 Then
1210                If .Invent_Skins.Object(.Invent_Skins.SlotBoatEquipped).ObjIndex = .Invent_Skins.ObjIndexBoatEquipped And .Invent_Skins.ObjIndexBoatEquipped > 0 Then
1220                    If CanEquipSkin(UserIndex, .Invent_Skins.SlotBoatEquipped, e_OBJType.otSkinsBoats, False) Then
1230                        Call SkinEquip(UserIndex, .Invent_Skins.SlotBoatEquipped, .Invent_Skins.Object(.Invent_Skins.SlotBoatEquipped).ObjIndex, e_OBJType.otSkinsBoats)
1240                    End If
1250                End If
1260            End If
1270        Else
1280            .invent.EquippedShipSlot = 0
1290        End If
1300    End If

        'Obtiene el indice-objeto municion
1310    If .invent.EquippedMunitionSlot > 0 Then
1320        If .invent.Object(.invent.EquippedMunitionSlot).ObjIndex > 0 Then
1330            .invent.EquippedMunitionObjIndex = .invent.Object(.invent.EquippedMunitionSlot).ObjIndex

1340        Else
1350            .invent.EquippedMunitionSlot = 0
1360        End If
1370    End If

        ' DM
1380    If .invent.EquippedRingAccesorySlot > 0 Then
1390        If .invent.Object(.invent.EquippedRingAccesorySlot).ObjIndex > 0 Then
1400            .invent.EquippedRingAccesoryObjIndex = .invent.Object(.invent.EquippedRingAccesorySlot).ObjIndex

1410            If .flags.Muerto = 0 Then
1420                .Char.DM_Aura = ObjData(.invent.EquippedRingAccesoryObjIndex).CreaGRH
1430            End If
1440        Else
1450            .invent.EquippedRingAccesorySlot = 0
1460        End If
1470    End If

1480    If .invent.EquippedAmuletAccesorySlot > 0 Then
1490        .invent.EquippedAmuletAccesoryObjIndex = .invent.Object(.invent.EquippedAmuletAccesorySlot).ObjIndex
1500        If ObjData(.invent.EquippedAmuletAccesoryObjIndex).CreaGRH <> "" Then
1510            .Char.Otra_Aura = ObjData(.invent.EquippedAmuletAccesoryObjIndex).CreaGRH
1520            Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageAuraToChar(.Char.charindex, .Char.Otra_Aura, False, 5))
1530        End If
1540        If ObjData(.invent.EquippedAmuletAccesoryObjIndex).Ropaje > 0 Then
1550            .Char.CartAnim = ObjData(.invent.EquippedAmuletAccesoryObjIndex).Ropaje
1560        End If
1570    End If

        ' RM
1580    If .invent.EquippedRingAccesorySlot > 0 Then
1590        If .invent.Object(.invent.EquippedRingAccesorySlot).ObjIndex > 0 Then
1600            .invent.EquippedRingAccesoryObjIndex = .invent.Object(.invent.EquippedRingAccesorySlot).ObjIndex

1610            If .flags.Muerto = 0 Then
1620                .Char.RM_Aura = ObjData(.invent.EquippedRingAccesoryObjIndex).CreaGRH
1630            End If
1640        Else
1650            .invent.EquippedRingAccesorySlot = 0
1660        End If
1670    End If

1680    If .invent.EquippedSaddleSlot > 0 Then
1690        If .invent.Object(.invent.EquippedSaddleSlot).ObjIndex > 0 Then
1700            .invent.EquippedSaddleObjIndex = .invent.Object(.invent.EquippedSaddleSlot).ObjIndex
1710        Else
1720            .invent.EquippedSaddleSlot = 0
1730        End If
1740    End If

1750    If .invent.EquippedWorkingToolSlot > 0 Then
1760        If .invent.Object(.invent.EquippedWorkingToolSlot).ObjIndex Then
1770            .invent.EquippedWorkingToolObjIndex = .invent.Object(.invent.EquippedWorkingToolSlot).ObjIndex
1780        Else
1790            .invent.EquippedWorkingToolSlot = 0
1800        End If
1810    End If

1820    If .invent.EquippedAmuletAccesorySlot > 0 Then
1830        If .invent.Object(.invent.EquippedAmuletAccesorySlot).ObjIndex Then
1840            .invent.EquippedAmuletAccesoryObjIndex = .invent.Object(.invent.EquippedAmuletAccesorySlot).ObjIndex

1850            If .flags.Muerto = 0 Then
1860                .Char.Otra_Aura = ObjData(.invent.EquippedAmuletAccesoryObjIndex).CreaGRH
1870            End If
1880        Else
1890            .invent.EquippedAmuletAccesorySlot = 0
1900        End If
1910    End If

1920    If .invent.EquippedShieldSlot = 0 Then .Char.ShieldAnim = NingunEscudo
1930    If .invent.EquippedHelmetSlot = 0 Then .Char.CascoAnim = NingunCasco
1940    If .invent.EquippedWeaponSlot = 0 And .invent.EquippedWorkingToolSlot = 0 Then .Char.WeaponAnim = NingunArma
1950    If .invent.EquippedAmuletAccesorySlot = 0 Then .Char.CartAnim = NoCart
        ' -----------------------------------------------------------------------
        '   FIN - INFORMACION INICIAL DEL PERSONAJE
        ' -----------------------------------------------------------------------

1960    If Not ValidateChr(UserIndex) Then
1970        Call WriteShowMessageBox(UserIndex, 1766, vbNullString)    'Msg1766=Error en el personaje. Comuniquese con el staff.
1980        Call CloseSocket(UserIndex)
1990        Exit Function

2000    End If

2010    .flags.SeguroParty = True
2020    .flags.SeguroClan = True
2030    .flags.SeguroResu = True
2040    .flags.LegionarySecure = True

2050    .CurrentInventorySlots = getMaxInventorySlots(UserIndex)

2060    Call WriteInventoryUnlockSlots(UserIndex)

2070    Call LoadUserIntervals(UserIndex)
2080    Call WriteIntervals(UserIndex)

2090    Call UpdateUserInv(True, UserIndex, 0)
2100    Call UpdateUserHechizos(True, UserIndex, 0)

2110    Call EnviarLlaves(UserIndex)

2120    If .flags.Paralizado Then Call WriteParalizeOK(UserIndex)

2130    If .flags.Inmovilizado Then Call WriteInmovilizaOK(UserIndex)

        ''
        'TODO : Feo, esto tiene que ser parche cliente
2140    If .flags.Estupidez = 0 Then
2150        Call WriteDumbNoMore(UserIndex)
2160    End If

2170    .flags.Inmunidad = 1
2180    .Counters.TiempoDeInmunidad = IntervaloPuedeSerAtacado

2190    .Counters.TiempoDeInmunidadParalisisNoMagicas = 0
2200    If MapInfo(.pos.Map).MapResource = 0 Then
2210        .pos.Map = Ciudades(.Hogar).Map
2220        .pos.x = Ciudades(.Hogar).x
2230        .pos.y = Ciudades(.Hogar).y
2240    End If

        'Mapa válido
2250    If Not MapaValido(.pos.Map) Then
2260        Call WriteErrorMsg(UserIndex, "EL PJ se encuenta en un mapa invalido.")
2270        Call CloseSocket(UserIndex)
2280        Exit Function
...
2300    If MapData(.pos.Map, .pos.x, .pos.y).UserIndex <> 0 Or MapData(.pos.Map, .pos.x, .pos.y).NpcIndex <> 0 Then
...
2310        FoundPlace = False
2320        esAgua = (MapData(.pos.Map, .pos.x, .pos.y).Blocked And FLAG_AGUA) <> 0

2330        For tY = .pos.y - 1 To .pos.y + 1
2340            For tX = .pos.x - 1 To .pos.x + 1

2350                If esAgua Then

                        'reviso que sea pos legal en agua, que no haya User ni NPC para poder loguear.
2360                    If LegalPos(.pos.Map, tX, tY, True, True, False, False, False) Then
2370                        FoundPlace = True
2380                        Exit For
...
2410                    If LegalPos(.pos.Map, tX, tY, False, True, False, False, False) Then
2420                        FoundPlace = True
2430                        Exit For
...
2470            If FoundPlace Then Exit For
2480        Next tY

2490        If FoundPlace Then    'Si encontramos un lugar, listo, nos quedamos ahi
...
2530            If MapData(.pos.Map, .pos.x, .pos.y).UserIndex <> 0 Then

                    'Si no encontramos lugar, y abajo teniamos a un usuario, lo pisamos y cerramos su comercio seguro
2540                If IsValidUserRef(UserList(MapData(.pos.Map, .pos.x, .pos.y).UserIndex).ComUsu.DestUsu) Then

                        'Le avisamos al que estaba comerciando que se tuvo que ir.
2550                    If UserList(UserList(MapData(.pos.Map, .pos.x, .pos.y).UserIndex).ComUsu.DestUsu.ArrayIndex).flags.UserLogged Then
2560                        Call FinComerciarUsu(UserList(MapData(.pos.Map, .pos.x, .pos.y).UserIndex).ComUsu.DestUsu.ArrayIndex)
2570                        Call WriteConsoleMsg(UserList(MapData(.pos.Map, .pos.x, .pos.y).UserIndex).ComUsu.DestUsu.ArrayIndex, PrepareMessageLocaleMsg(1925, vbNullString, e_FontTypeNames.FONTTYPE_WARNING))    ' Msg1925=Comercio cancelado. El otro usuario se ha desconectado.
2580                    End If

                        'Lo sacamos.
2590                    If UserList(MapData(.pos.Map, .pos.x, .pos.y).UserIndex).flags.UserLogged Then
2600                        Call FinComerciarUsu(MapData(.pos.Map, .pos.x, .pos.y).UserIndex)
2610                        Call WriteErrorMsg(MapData(.pos.Map, .pos.x, .pos.y).UserIndex, "Alguien se ha conectado donde te encontrabas, por favor reconectate...")

2620                    End If

2630                End If

2640                Call CloseSocket(MapData(.pos.Map, .pos.x, .pos.y).UserIndex)
...
2680    trigger = MapData(.pos.Map, .pos.x, .pos.y).trigger

2690    If trigger = e_Trigger.DETALLEAGUA Then    'Esta en zona de caucho obj 199, 200

2700        If .raza = e_Raza.Enano Or .raza = e_Raza.Gnomo Then
2710            itemBuscado = iObjTrajeBajoNw
2720        Else
2730            itemBuscado = iObjTrajeAltoNw
2740        End If
2750        slotBarco = GetSlotInInventory(UserIndex, itemBuscado)
2760        If slotBarco > -1 Then
2770            .invent.EquippedShipObjIndex = itemBuscado
2780            .invent.EquippedShipSlot = slotBarco
2790        End If

2800    ElseIf trigger = e_Trigger.VALIDONADO Or trigger = e_Trigger.NADOCOMBINADO Then    'Esta en zona de nado comun obj 197

2810        itemBuscado = iObjTraje
2820        slotBarco = GetSlotInInventory(UserIndex, itemBuscado)
2830        If slotBarco > -1 Then
2840            .invent.EquippedShipObjIndex = itemBuscado
2850            .invent.EquippedShipSlot = slotBarco
2860        End If
2870    End If

2880    If .invent.EquippedShipObjIndex > 0 And (MapData(.pos.Map, .pos.x, .pos.y).Blocked And FLAG_AGUA) <> 0 Then
2890        .flags.Navegando = 1
2900        Call EquiparBarco(UserIndex)
2910    ElseIf .flags.Navegando = 1 And (MapData(.pos.Map, .pos.x, .pos.y).Blocked And FLAG_AGUA) <> 0 Then
            Dim iSlot           As Integer
2920        For iSlot = 1 To UBound(.invent.Object)
2930            If .invent.Object(iSlot).ObjIndex > 0 Then
2940                If ObjData(.invent.Object(iSlot).ObjIndex).OBJType = otShips And ObjData(.invent.Object(iSlot).ObjIndex).Subtipo > 0 Then
2950                    .invent.EquippedShipObjIndex = .invent.Object(iSlot).ObjIndex
2960                    .invent.EquippedShipSlot = iSlot
2970                    Exit For
2980                End If
2990            End If
3000        Next
...
3020    If .invent.EquippedAmuletAccesoryObjIndex <> 0 Then
3030        If ObjData(.invent.EquippedAmuletAccesoryObjIndex).EfectoMagico = 11 Then .flags.Paraliza = 1
3040    End If

3050    Call WriteUserIndexInServer(UserIndex)    'Enviamos el User index

3060    Call WriteHora(UserIndex)
3070    Call WriteChangeMap(UserIndex, .pos.Map)    'Carga el mapa
3080    Call UpdateCharWithEquipedItems(UserIndex)
3090    Select Case .flags.Privilegios

            Case e_PlayerType.Admin
3100            .flags.ChatColor = RGB(252, 195, 0)
3110        Case e_PlayerType.Dios
3120            .flags.ChatColor = RGB(26, 209, 107)
3130        Case e_PlayerType.SemiDios
3140            .flags.ChatColor = RGB(60, 150, 60)
3150        Case e_PlayerType.Consejero
3160            .flags.ChatColor = RGB(170, 170, 170)
3170        Case Else
3180            .flags.ChatColor = vbWhite
3190    End Select

3200    Select Case .Faccion.Status
            Case e_Facciones.Ciudadano
3210            .flags.ChatColor = vbWhite
3220        Case e_Facciones.Armada
3230            .flags.ChatColor = vbWhite
3240        Case e_Facciones.consejo
3250            .flags.ChatColor = RGB(66, 201, 255)
3260        Case e_Facciones.Criminal
3270            .flags.ChatColor = vbWhite
3280        Case e_Facciones.Caos
3290            .flags.ChatColor = vbWhite
3300        Case e_Facciones.concilio
3310            .flags.ChatColor = RGB(255, 102, 102)
3320    End Select

        ' Jopi: Te saco de los mapas de retos (si logueas ahi) 324 372 389 390
3330    If Not EsGM(UserIndex) And (.pos.Map = 324 Or .pos.Map = 372 Or .pos.Map = 389 Or .pos.Map = 390) Then

            ' Si tiene una posicion a la que volver, lo mando ahi
3340        If MapaValido(.flags.ReturnPos.Map) And .flags.ReturnPos.x > 0 And .flags.ReturnPos.x <= XMaxMapSize And .flags.ReturnPos.y > 0 And .flags.ReturnPos.y <= YMaxMapSize Then

3350            Call WarpToLegalPos(UserIndex, .flags.ReturnPos.Map, .flags.ReturnPos.x, .flags.ReturnPos.y, True)

3360        Else               ' Lo mando a su hogar

3370            Call WarpToLegalPos(UserIndex, Ciudades(.Hogar).Map, Ciudades(.Hogar).x, Ciudades(.Hogar).y, True)
...
3410    Call MakeUserChar(True, .pos.Map, UserIndex, .pos.Map, .pos.x, .pos.y, 1)

3420    Call WriteUserCharIndexInServer(UserIndex)
3430    Call ActualizarVelocidadDeUsuario(UserIndex)

3440    If .flags.Privilegios And (e_PlayerType.SemiDios Or e_PlayerType.Dios Or e_PlayerType.Admin) Then
3450        Call DoAdminInvisible(UserIndex)
3460    End If

3470    Call WriteUpdateUserStats(UserIndex)
3480    Call WriteUpdateHungerAndThirst(UserIndex)
3490    Call WriteUpdateDM(UserIndex)
3500    Call WriteUpdateRM(UserIndex)

3510    Call SendMOTD(UserIndex)

        'Actualiza el Num de usuarios
3520    NumUsers = NumUsers + 1
3530    .flags.UserLogged = True
3540    Call Execute("Update user set is_logged = true where id = ?", UserList(UserIndex).Id)
3550    .Counters.LastSave = GetTickCount

3560    MapInfo(.pos.Map).NumUsers = MapInfo(.pos.Map).NumUsers + 1

3570    If .Stats.SkillPts > 0 Then
3580        Call WriteSendSkills(UserIndex)
3590        Call WriteLevelUp(UserIndex, .Stats.SkillPts)

3600    End If

3610    If NumUsers > DayStats.MaxUsuarios Then DayStats.MaxUsuarios = NumUsers

3620    If NumUsers > RecordUsuarios Then
3630        Call SendData(SendTarget.ToAll, 0, PrepareMessageLocaleMsg("1550", NumUsers, e_FontTypeNames.FONTTYPE_INFO))    ' Msg1550=Record de usuarios conectados simultáneamente: ¬1 usuarios.
3640        RecordUsuarios = NumUsers
3650    End If

3660    Call SendData(SendTarget.ToIndex, UserIndex, PrepareMessageOnlineUser(NumUsers))

3670    Call WriteFYA(UserIndex)
3680    Call WriteBindKeys(UserIndex)



3690    If .NroMascotas > 0 And MapInfo(.pos.Map).NoMascotas = 0 And .flags.MascotasGuardadas = 0 Then

3700        For i = 1 To MAXMASCOTAS
3710            If .MascotasType(i) > 0 Then
3720                Call SetNpcRef(.MascotasIndex(i), SpawnNpc(.MascotasType(i), .pos, False, False, False, UserIndex))
3730                If .MascotasIndex(i).ArrayIndex > 0 Then
3740                    Call SetUserRef(NpcList(.MascotasIndex(i).ArrayIndex).MaestroUser, UserIndex)
3750                    Call FollowAmo(.MascotasIndex(i).ArrayIndex)
3760                End If
3770            End If
3780        Next i
3790    End If

3800    If .flags.Montado = 1 Then
3810        Call WriteEquiteToggle(UserIndex)
3820    End If

3830    Call ActualizarVelocidadDeUsuario(UserIndex)

3840    If .GuildIndex > 0 Then

            'welcome to the show baby...
3850        If Not modGuilds.m_ConectarMiembroAClan(UserIndex, .GuildIndex) Then
                ' Msg521=Tu estado no te permite entrar al clan.
3860            Call WriteLocaleMsg(UserIndex, "521", e_FontTypeNames.FONTTYPE_GUILD)
3870        End If

3880    End If

3890    If LenB(.LastGuildRejection) <> 0 Then
3900        Call WriteShowMessageBox(UserIndex, 1767, .LastGuildRejection)    'Msg1767=Tu solicitud de ingreso al clan ha sido rechazada. El clan te explica que: ¬1
3910        .LastGuildRejection = vbNullString

3920        Call SaveUserGuildRejectionReason(.name, vbNullString)
3930    End If

3940    If Lloviendo Then Call WriteRainToggle(UserIndex)

3950    If ServidorNublado Then Call WriteNubesToggle(UserIndex)

3960    Call WriteLoggedMessage(UserIndex, newUser)

3970    If .Stats.ELV = 1 Then
3980        Call WriteLocaleMsg(UserIndex, "522", e_FontTypeNames.FONTTYPE_GUILD, .name)    ' Msg522=¡Bienvenido a las tierras de Argentum Online! ¡<nombre> que tengas buen viaje y mucha suerte!
3990    Else
4000        Call WriteLocaleMsg(UserIndex, "1439", e_FontTypeNames.FONTTYPE_GUILD, .name & "¬" & .Stats.ELV & "¬" & get_map_name(.pos.Map))    ' Msg1439=¡Bienvenido de nuevo ¬1! Actualmente estas en el nivel ¬2 en ¬3, ¡buen viaje y mucha suerte!
4010    End If

4020    If Status(UserIndex) = e_Facciones.Criminal Or Status(UserIndex) = e_Facciones.Caos Or Status(UserIndex) = e_Facciones.concilio Then
4030        Call WriteSafeModeOff(UserIndex)
4040        .flags.Seguro = False
4050        Call WriteLegionarySecure(UserIndex, False)
4060        .flags.LegionarySecure = False
4070    Else
4080        .flags.Seguro = True
4090        Call WriteSafeModeOn(UserIndex)

4100    End If

4110    If LenB(.MENSAJEINFORMACION) > 0 Then
            Dim Lines()         As String
4120        Lines = Split(.MENSAJEINFORMACION, vbNewLine)

4130        For i = 0 To UBound(Lines)

4140            If LenB(Lines(i)) > 0 Then
4150                Call WriteConsoleMsg(UserIndex, Lines(i), e_FontTypeNames.FONTTYPE_New_DONADOR)
4160            End If

4170        Next

4180        .MENSAJEINFORMACION = vbNullString
4190    End If

4200    If EventoActivo Then
4210        Call WriteLocaleMsg(UserIndex, 1625, e_FontTypeNames.FONTTYPE_New_Eventos, PublicidadEvento & "¬" & TiempoRestanteEvento)    'Msg1625=¬1. Tiempo restante: ¬2 minuto(s).
4220    End If

4230    Call WriteContadores(UserIndex)
4240    Call WritePrivilegios(UserIndex)
4250    Call RestoreDCUserCache(UserIndex)
4260    Call CustomScenarios.UserConnected(UserIndex)
4270    Call AntiCheat.OnNewPlayerConnect(UserIndex)
4280 End With


4290 ConnectUser_Complete = True

4300 Exit Function

Complete_ConnectUser_Err:
4310 Call TraceError(Err.Number, Err.Description, "UsUaRiOs.ConnectUser_Complete", Erl)
