            ...
            For i = 1 To .Invent_Skins.count
                If .Invent_Skins.Object(i).ObjIndex > 0 Then
                    If Not Database_Queries.Exists("inventory_item_skins", "user_id", CStr(.Id), "skin_id", .Invent_Skins.Object(i).ObjIndex) Then
                        sQuery.Append "INSERT INTO inventory_item_skins (user_id, skin_id, type_skin, skin_equipped) Values (" & .Id & "," & .Invent_Skins.Object(i).ObjIndex & _
                                "," & ObjData(.Invent_Skins.Object(i).ObjIndex).OBJType & "," & .Invent_Skins.Object(i).Equipped & ")"
                        Database.Execute sQuery.ToString
                        sQuery.Clear
                    Else
                        sQuery.Append "UPDATE inventory_item_skins SET SKIN_EQUIPPED=" & IIf(.Invent_Skins.Object(i).Equipped, "1", "0") & " WHERE USER_ID=" & .Id & _
                                " AND SKIN_ID=" & .Invent_Skins.Object(i).ObjIndex
                        Database.Execute sQuery.ToString
                        sQuery.Clear
                    End If
                End If
